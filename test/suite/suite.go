...
func register(ctx context.Context, mgr manager.Manager, s *ctrltest.Suite) error {
	...
	go (&poolerSim{c: s.Client, rpc: rpc, topo: topo, interval: 500 * time.Millisecond}).run(
		ctx,
	)
	...
	// production would silently turn every ordering assertion in the seam
	// tests into a race that fails a few times a week and reads as operator
