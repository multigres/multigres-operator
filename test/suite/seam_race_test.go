...
// stale watch.
const shardProbeAnnotation = "seam-race-test.multigres.com/probe"
...
func TestTwoControllersWriteOneShard(t *testing.T) {
	...
	cluster := MinimalCluster(t, ns, "seam")
	...
	t.Run("field ownership on the Shard is disjoint", func(t *testing.T) {
		requireDisjointFieldOwnership(t, key)
	})

	t.Run("tablegroup's patches to the Shard never change it", func(t *testing.T) {
		requireTableGroupPatchesAreNoOps(t, ns, key)
	})
}

// requireDisjointFieldOwnership is assertion 3 from the brief, and the one that
// generalises: it is the check that would have caught the status hot-loop
// directly; the loop was two field managers each asserting a value for the same
// field, ObservedGeneration or a phase, and the API server obliging both. A
// dump of writes only shows that as churn; managedFields shows it as the
// overlap it is.
//
...
// the same library the server itself uses and asserts that no field path is a
// member of more than one.
func requireDisjointFieldOwnership(t *testing.T, key client.ObjectKey) {
	t.Helper()

	shard := &multigresv1alpha1.Shard{}
	if err := Suite.Client.Get(t.Context(), key, shard); err != nil {
		t.Fatalf("get shard %s: %v", key, err)
	}
	...
	for _, mf := range shard.ManagedFields {
		...
		if err := set.FromJSON(mf.FieldsV1.GetRawReader()); err != nil {
			t.Fatalf("decode managedFields for manager %s: %v", mf.Manager, err)
		}
...
func requireTableGroupPatchesAreNoOps(t *testing.T, ns string, key client.ObjectKey) {
	t.Helper()

	const passesToObserve = 5
	for i := range passesToObserve {
		before := &multigresv1alpha1.Shard{}
		if err := Suite.Client.Get(t.Context(), key, before); err != nil {
			t.Fatalf("get shard %s: %v", key, err)
		}

		probedFrom := probeShard(t, key, i)
		pass := awaitTableGroupPassAfter(t, ns, key, probedFrom)

		after := &multigresv1alpha1.Shard{}
		if err := Suite.Client.Get(t.Context(), key, after); err != nil {
			t.Fatalf("get shard %s: %v", key, err)
		}
		...
		if !equality.Semantic.DeepEqual(before.OwnerReferences, after.OwnerReferences) {
			t.Fatalf(
		...
		if !equality.Semantic.DeepEqual(before.Labels, after.Labels) {
			t.Fatalf(
...
func awaitTableGroupPassAfter(
	...
	ctrltest.Eventually(t, 10*time.Second, what, func() error {
...
func probeShard(t *testing.T, key client.ObjectKey, seq int) time.Time {
	t.Helper()
	shard := &multigresv1alpha1.Shard{}
	if err := Suite.Client.Get(t.Context(), key, shard); err != nil {
		t.Fatalf("get shard %s: %v", key, err)
	...
	issued := time.Now()
	if err := Suite.Client.Patch(t.Context(), shard, client.MergeFrom(base)); err != nil {
		t.Fatalf("annotate shard %s: %v", key, err)
	}
	return issued
}
