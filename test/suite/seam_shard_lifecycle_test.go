...
func lifecycleShardRef(clusterName string) *multigresv1alpha1.Shard {
...
func lifecycleCluster(t *testing.T, ns, clusterName string) *multigresv1alpha1.MultigresCluster {
	t.Helper()

	scName := lifecycleStorageClassName(ns)
	allowExpansion := true
	...
	if err := Suite.Client.Create(t.Context(), sc); err != nil {
		t.Fatalf("create StorageClass %s: %v", scName, err)
	}
	...
	// t.Context, which is already cancelled by the time cleanups run.
	t.Cleanup(func() {
		_ = Suite.Client.Delete(context.Background(), sc)
	})
...
func updateLifecyclePool(
	t *testing.T,
	ns, clusterName string,
	mutate func(*multigresv1alpha1.PoolSpec),
) {
	t.Helper()
	key := client.ObjectKey{Namespace: ns, Name: clusterName}
	for {
		cluster := &multigresv1alpha1.MultigresCluster{}
		if err := Suite.Client.Get(t.Context(), key, cluster); err != nil {
			t.Fatalf("get cluster %s: %v", clusterName, err)
		}
		...
		err := Suite.Client.Update(t.Context(), cluster)
		if err == nil {
			return
		}
		if !apierrors.IsConflict(err) {
			t.Fatalf("update cluster %s: %v", clusterName, err)
		}
	}
}
...
func drainStateSeen(t *testing.T, ns string) error {
	t.Helper()
	pods := &corev1.PodList{}
	if err := Suite.Client.List(t.Context(), pods, client.InNamespace(ns)); err != nil {
		return err
	}
	...
	return fmt.Errorf("no pod in %s carries %s", ns, metadata.AnnotationDrainState)
}
...
func poolPodFingerprints(t *testing.T, ns string) map[string]string {
	t.Helper()
	pods := &corev1.PodList{}
	if err := Suite.Client.List(t.Context(), pods, client.InNamespace(ns)); err != nil {
...
func TestShardLifecycle(t *testing.T) {
	...
	lifecycleCluster(t, ns, clusterName)
	ctrltest.Eventually(t, 60*time.Second, "shard to report Healthy", func() error {
		shard := &multigresv1alpha1.Shard{}
		if err := Suite.Client.Get(t.Context(), shardKey, shard); err != nil {
			return err
		}
	...
	Suite.RequireQuiescent(t, ns, 5*time.Second, 60*time.Second)
	...
	s := Suite.NewScript(t, ns, &corev1.PersistentVolumeClaimList{})
	...
	checkPodRoles := func() error {
		members, err := MembersOf(context.Background(), Suite.Client, shardKey)
		if err != nil {
	...
	requirePodRoles := func(where string) {
		t.Helper()
		if err := checkPodRoles(); err != nil {
			t.Errorf("pod roles after %s: %v", where, err)
		}
	}
	s.Invariant(
	...
	converged := poolPodFingerprints(t, ns)
	for _, want := range []string{pod0, pod1} {
		if _, ok := converged[want]; !ok {
			t.Fatalf("converged pool has no pod %s, only %v", want, converged)
		}
	}
	if len(converged) != 2 {
		t.Fatalf("converged pool has %d pods, want exactly %s and %s: %v",
			len(converged), pod0, pod1, converged)
	}
	...
	initialMembers, err := MembersOf(t.Context(), Suite.Client, shardKey)
	if err != nil {
		t.Fatalf("MembersOf before step 2: %v", err)
	}
	if len(initialMembers.Replicas) != 1 || initialMembers.Primary == "" {
		t.Fatalf("want one primary and one replica after step 1, got %+v", initialMembers)
	}
	...
	s.Step("change CPU and memory on the pool", func() error {
		updateLifecyclePool(t, ns, clusterName, func(p *multigresv1alpha1.PoolSpec) {
		...
		podsBefore := poolPodFingerprints(t, ns)
		ctrltest.KnownDefect(t, "shard-lifecycle-two-member-pool-never-rolls",
			func() error {
				return waitFor(blockedObservationWindow, func() error {
					return drainStateSeen(t, ns)
				})
			},
		)
		...
		if diff := podFingerprintDiff(podsBefore, poolPodFingerprints(t, ns)); diff != "" {
			t.Errorf("the refused rolling update still touched pods: %s", diff)
		}
	...
	s.Step("add a replica", func() error {
		...
		ctrltest.Eventually(t, 30*time.Second, "new pod ready", func() error {
			pod := &corev1.Pod{}
			key := client.ObjectKey{Namespace: ns, Name: pod2}
			if err := Suite.Client.Get(t.Context(), key, pod); err != nil {
				return err
			}
	...
	switch {
	case !pvcCreated:
		t.Errorf("the shard controller never created PVC %s", pvc2)
	case !podCreated:
		t.Errorf("the shard controller never created pod %s", pod2)
	case pvcAt > podAt:
		t.Errorf("pod %s was created before the PVC %s it binds (ops %d and %d)",
	...
	// above.
	ctrltest.Eventually(t, 30*time.Second, "new pod bound to a PVC", func() error {
		_, err := ShardPVCOf(t.Context(), Suite.Client, ns, pod2)
		return err
	})
	boundTo, err := ShardPVCOf(t.Context(), Suite.Client, ns, pod2)
	if err != nil {
		t.Fatalf("ShardPVCOf(%s): %v", pod2, err)
	}
	...
	if boundTo != pvc2 {
		t.Fatalf("pod %s is bound to PVC %s, want the new PVC %s rather than %s or %s",
			pod2, boundTo, pvc2, pvc0, pvc1)
	}
	...
	members, err := MembersOf(t.Context(), Suite.Client, shardKey)
	if err != nil {
		t.Fatalf("MembersOf before step 5: %v", err)
	}
	if len(members.Replicas) != 2 || members.Primary == "" {
		t.Fatalf("want one primary and two replicas before step 5, got %+v", members)
	}
	...
	membersBeforeScaleDown, err := MembersOf(t.Context(), Suite.Client, shardKey)
	if err != nil {
		t.Fatalf("MembersOf before step 6: %v", err)
	}
	if len(membersBeforeScaleDown.Replicas) != 2 || membersBeforeScaleDown.Primary == "" {
		t.Fatalf("want one primary and two replicas before step 6, got %+v",
			membersBeforeScaleDown)
	}
	...
	s.Step("scale the pool back down by one replica", func() error {
		...
		if diff := podFingerprintDiff(podsBefore, poolPodFingerprints(t, ns)); diff != "" {
			t.Errorf("the refused scale-down still touched pods: %s", diff)
		}
