...
func TestExternalGateway_EnableDisableLifecycle(t *testing.T) {
	...
	c.Require().
		NoError(watcher.WaitForMatch(expectedGwSvc), "global multigateway Service should be ClusterIP with externalIPs and annotations")
	c.Require().
		NoError(watcher.WaitForMatch(expectedReplicaGwSvc), "global multigateway replica Service should be ClusterIP")
	...
	}, "condition should be False/NoReadyGateways before gateways are ready")

	// Step 4: Simulate Cell reporting ready gateways by updating Cell status.
	var cellList multigresv1alpha1.CellList
	c.Require().NoError(k8sClient.List(t.Context(), &cellList,
		client.InNamespace(testNamespace),
		client.MatchingLabels{"multigres.com/cluster": clusterName},
	))
	c.Require().NotEmpty(cellList.Items, "expected at least one Cell CR")
	...
	c.Require().
		NoError(k8sClient.Status().Update(t.Context(), cell), "simulating Cell reporting ready gateway replicas")
	...
	c.Require().NoError(k8sClient.Update(t.Context(), cluster), "disabling external gateway")
	...
	c.Require().
		NoError(watcher.WaitForMatch(expectedClusterIPSvc), "global multigateway Service should revert to ClusterIP after disabling")
	c.Require().
		NoError(watcher.WaitForMatch(expectedReplicaClusterIPSvc), "global multigateway replica Service should remain ClusterIP")
	...
	}, "gateway status should be nil and condition removed after disabling")
}
...
func TestExternalGateway_NoReadyGatewaysTransition(t *testing.T) {
	...
	}, "condition should be False/NoReadyGateways with endpoint populated")
	...
	cell.Status.ObservedGeneration = cell.Generation
	c.Require().NoError(k8sClient.Status().Update(t.Context(), cell))
	...
	c.EventuallyTrue(testTimeout, pollInterval, func() bool {
		...
		return cond != nil &&
			...
			mgc.Status.Gateway != nil &&
			mgc.Status.Gateway.ExternalEndpoint == "2001:db8::101"
	}, "condition should transition to True/EndpointReady after Cell reports ready gateways")
