...
func TestLoadClientTLS_PlaintextWhenNoSecrets(t *testing.T) {
	...
	c.Require().NoError(err, "loadClientTLS() error =")
	c.Nil(opts, "expected nil TLS options for a plaintext reference, got")
}
...
func TestLoadClientTLS_ManagedSecret(t *testing.T) {
	...
	c.Require().NoError(err, "loadClientTLS() error =")
	c.Require().NotNil(opts, "expected TLS options, got nil")
	c.False(!bytes.Equal(opts.CertPEM, []byte("CERT")) ||
		!bytes.Equal(opts.KeyPEM, []byte("KEY")) ||
		!bytes.Equal(
			opts.CAPEM,
			[]byte("CA"),
		), "TLS options do not carry the Secret material verbatim: %+v", opts)
}
...
func TestLoadClientTLS_SeparateCAAndClientSecrets(t *testing.T) {
	...
	c.Require().NoError(err, "loadClientTLS() error =")
	c.False(
		!bytes.Equal(opts.CertPEM, []byte("CERT")) || !bytes.Equal(opts.CAPEM, []byte("CA")),
		"expected material from both Secrets: %+v",
		opts,
	)
}
